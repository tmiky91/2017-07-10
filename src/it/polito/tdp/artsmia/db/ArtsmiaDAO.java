...
import it.polito.tdp.artsmia.model.Esibizione;
...
public class ArtsmiaDAO {
	...
	public List<Esibizione> getEsibizioniComuni(Map<Integer, ArtObject> idMap) {
		final String sql=	"select eo1.object_id as id1, eo2.object_id as id2, count(*) as peso " + 
							...
							"group by id1, id2";
		List<Esibizione> result = new LinkedList<>();
		Connection conn = DBConnect.getConnection();
		...
		try {
			...
			while (res.next()) {
				...
				if(o1!=null && o2!=null) {
					Esibizione esibizione = new Esibizione(o1, o2, res.getDouble("peso"));
					result.add(esibizione);
				}
			}
			conn.close();
			return result;
